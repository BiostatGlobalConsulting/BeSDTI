...
# export_table_to_excel R version 1.00 - Biostat Global Consulting - 2024-08-08
# *******************************************************************************
# Change log

# Date 			  Version 	Name			      What Changed
# 2024-08-08  1.00      Caitlin Clary   Original R version adapted from v1.07 of
#                                       export_table_to_excel in vcqiR
...
export_table_to_excel <- function(indicator, tablename = NULL, sheet = NULL,
  ...
  if (file.exists(paste0(OUTPUT_FOLDER, "/", ANALYSIS_NAME, "_TO.xlsx"))){
    wb <- loadWorkbook(paste0(OUTPUT_FOLDER, "/", ANALYSIS_NAME, "_TO.xlsx"))
    sheetnames <- getSheetNames(paste0(OUTPUT_FOLDER, "/", ANALYSIS_NAME, "_TO.xlsx"))
  ...
  if (besd_object_exists(paste0(indicator, "_TO_TITLE"))){
  ...
  if (besd_object_exists(paste0(indicator, "_TO_SUBTITLE"))){
  ...
  i = 1
  while (besd_object_exists(paste0(indicator, "_TO_FOOTNOTE_", i))){
  ...
  if (use_basic_fmtids == 0){
    for (l in 1:nrow(cellformat)){
      ...
      # Only go through lines that aren't blank rows
  ...
  format1 <- createStyle(numFmt = paste0("0.", rep(0, NUM_DECIMAL_DIGITS)))
  format2 <- createStyle(numFmt = "0.0000")
  format3 <- createStyle(numFmt = "#,##0")
  format4 <- createStyle(numFmt = "0.0")

  if (length(format1col) > 0){
    addStyle(wb, sheet = sheet1, style = format1, rows = startrow:(startrow+nrows),
             cols = format1col, gridExpand = TRUE, stack = TRUE)
  }

  if (length(format2col) > 0){
    addStyle(wb, sheet = sheet1, style = format2, rows = startrow:(startrow+nrows),
             cols = format2col, gridExpand = TRUE, stack = TRUE)
  }

  if (length(format3col) > 0){
    addStyle(wb, sheet = sheet1, style = format3, rows = startrow:(startrow+nrows),
             cols = format3col, gridExpand = TRUE, stack = TRUE)
  }

  if (length(format4col) > 0){
    addStyle(wb, sheet = sheet1, style = format4, rows = startrow:(startrow+nrows),
             cols = format4col, gridExpand = TRUE, stack = TRUE)
  }
  ...
  if (length(format5col) > 0){
    ...
    for (i in seq_along(format5col)){
      ...
      addStyle(wb, sheet = sheet1, style = format5, rows = startrow:(startrow+nrows),
               cols = format5col[i], gridExpand = TRUE, stack = TRUE)
    }
  }
  ...
  if (brief == TRUE){
    ...
    brieftb <- brieftb %>% arrange(outputid) %>% select(-c(outputid))
    ...
    if (length(format1col) > 0){
      addStyle(wb, sheet = sheet2, style = format1, rows = startrow:(startrow+nrows),
               cols = format1col, gridExpand = TRUE, stack = TRUE)
    }

    if (length(format2col) > 0){
      addStyle(wb, sheet = sheet2, style = format2, rows = startrow:(startrow+nrows),
               cols = format2col, gridExpand = TRUE, stack = TRUE)
    }

    if (length(format3col) > 0){
      addStyle(wb, sheet = sheet2, style = format3, rows = startrow:(startrow+nrows),
               cols = format3col, gridExpand = TRUE, stack = TRUE)
    }

    if (length(format4col) > 0){
      addStyle(wb, sheet = sheet2, style = format4, rows = startrow:(startrow+nrows),
               cols = format4col, gridExpand = TRUE, stack = TRUE)
    }
    ...
    if (length(format5col) > 0){
      ...
      for (i in seq_along(format5col)){
        ...
        format5 <- createStyle(numFmt = currentformat[[1]])
        addStyle(wb, sheet = sheet2, style = format5, rows = startrow:(startrow+nrows),
                 cols = format5col[i], gridExpand = TRUE, stack = TRUE)
      }
    }
  ...
  saveWorkbook(wb, file = paste0(OUTPUT_FOLDER, "/", ANALYSIS_NAME, "_TO.xlsx"), overwrite = TRUE)
