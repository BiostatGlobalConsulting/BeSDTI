#' Check global macros for BESD_DESC_02
#'
...
# BESD_DESC_02_00GC R version 1.00 - Biostat Global Consulting - 2024-08-08
# *******************************************************************************
# Change log

# Date 			  Version 	Name			      What Changed
# 2024-08-08  1.00      Caitlin Clary   Original R version adapted from v1.01 of
#                                       DESC_02_00GC in vcqiR
# *******************************************************************************

BESD_DESC_02_00GC <- function(VCP = "BESD_DESC_02_00GC"){
  besd_log_comment(VCP, 5, "Flow", "Starting")

  exitflag <- 0
  errormsgs <- NULL

  if (!besd_object_exists("DESC_02_DATASET")){
    errormsgs <- c(errormsgs, "You must specify DESC_02_DATASET.")
    besd_log_comment(VCP, 1, "Error", "You must specify DESC_02_DATASET.")
    exitflag <- 1
  }
  ...
  if (!file.exists(paste0(OUTPUT_FOLDER, "/", DESC_02_DATASET))){
    errormsgs <- c(errormsgs,
                   paste0(OUTPUT_FOLDER, "/", DESC_02_DATASET," does not exist."))
    besd_log_comment(VCP, 1, "Error",
                     paste0(OUTPUT_FOLDER, "/", DESC_02_DATASET," does not exist."))
    exitflag <- 1
  } else {
    dat <- besd_read(file = paste0(OUTPUT_FOLDER, "/", DESC_02_DATASET))
  }
  ...
  if (!besd_object_exists("DESC_02_VARIABLES")){
    ...
    besd_log_comment(VCP, 1, "Error", "You must define global variable DESC_02_VARIABLES")
    exitflag <- 1
  }
  ...
  # R only:
  if (besd_object_exists("DESC_02_SHOW_SUBTOTALS_ONLY")){
  ...
  if (!besd_object_exists("DESC_02_N_LABEL")){
    besd_global(DESC_02_N_LABEL, language_string(language_use = language_use, str = "OS_48")) #N
  }
  if (!besd_object_exists("DESC_02_NWTD_LABEL")){
    besd_global(DESC_02_NWTD_LABEL, language_string(language_use = language_use, str = "OS_323")) #Weighted N
  }
  ...
  if ((DESC_02_N_RELABEL_LEVELS == 0) %in% TRUE){
    if (besd_object_exists("DESC_02_RELABEL_LEVEL_1")){
      ...
      besd_log_comment(VCP, 2, "Warning", "Warning: Global macro DESC_02_RELABEL_LEVEL_1 is defined,",
  ...
  if ((DESC_02_N_RELABEL_LEVELS != 0) %in% TRUE){
    for (i in 1:DESC_02_N_RELABEL_LEVELS){
      ...
      if ((!besd_object_exists(paste0("DESC_02_RELABEL_LEVEL_", i)) & !is.na(relab_level)) %in% TRUE){
      ...
      if (!besd_object_exists(paste0("DESC_02_RELABEL_LABEL_", i))){
  ...
  if ((DESC_02_N_SUBTOTALS == 0) %in% TRUE){
    if (besd_object_exists("DESC_02_SUBTOTAL_LEVELS_1")){
      ...
      besd_log_comment(VCP, 2, "Warning: Global macro DESC_02_SUBTOTAL_LEVELS_1 is defined, but DESC_02_N_SUBTOTALS is 0 or not defined, so no subtotals will be calculated.")
    }
  }
  ...
  if ((DESC_02_N_SUBTOTALS != 0) %in% TRUE){
    for (i in 1:DESC_02_N_SUBTOTALS){
      ...
      if(!besd_object_exists(paste0("DESC_02_SUBTOTAL_LABEL_", i))){
      ...
      if (besd_object_exists(paste0("DESC_02_SUBTOTAL_LIST_", i))){
        ...
        if (!str_to_upper(word(sub_list,1)) %in% c("BEFORE", "AFTER")){
          errormsgs <- c(
            errormsgs, paste0("The first word of global DESC_02_SUBTOTAL_LIST_", i,
      ...
      # For each value provided in SUBTOTAL_LIST confirm it is an appropriate
      # value for the variable provided
      for (v in seq_along(DESC_02_VARIABLES)){
        ...
        if (class(var2) == "character"){
          errormsgs <- c(
            errormsgs,
            paste0("DESC_02 can only compute subtotals for numeric variables; ",
                   DESC_02_VARIABLES[v],
    ...
    # Confirm that if "${DESC_02_SHOW_SUBTOTALS_ONLY"}" then the user has
		# not also specified any SUBTOTAL_LEVEL globals

    if (besd_object_exists("DESC_02_SHOW_SUBTOTALS_ONLY")){
      for (i in 1: DESC_02_N_SUBTOTALS){
        if (besd_object_exists(paste0("DESC_02_SUBTOTAL_LIST_", i))){
          warning(paste0("If you specify DESC_02_SHOW_SUBTOTALS_ONLY then you cannot also specify SUBTOTAL_LIST; DESC_02_SUBTOTAL_LIST_",
                         i, " will be ignored."))
          besd_log_comment(VCP,2,"Warning",
                           paste0("If you specify DESC_02_SHOW_SUBTOTALS_ONLY then you cannot also specify SUBTOTAL_LIST; DESC_02_SUBTOTAL_LIST_",
                                  i, " will be ignored."))
          # Clear out SUBTOTAL_LIST global
          rm(list = c(paste0(paste0("DESC_02_SUBTOTAL_LIST_", i))))
          besd_log_comment(VCP, 3, "Global", paste0("Global value DESC_02_SUBTOTAL_LIST_", i, " has been removed."))
        }
      } # end of DESC_02_N_SUBTOTALS i loop
    }
  }
