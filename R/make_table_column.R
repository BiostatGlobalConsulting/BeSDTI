...
# make_table_column R version 1.00 - Biostat Global Consulting - 2024-09-09
# *******************************************************************************
# Change log

# Date 			  Version 	Name			      What Changed
# 2024-08-08  1.00      Caitlin Clary   Original R version adapted from v1.07 of
#                                       make_table_column in vcqiR
# *******************************************************************************
...
make_table_column <- function(
  ...
  if (!exists(tablename, envir = .GlobalEnv) | !exists(paste0(tablename, "_CN"), envir = .GlobalEnv)){
    temptable <- besd_read(paste0(OUTPUT_FOLDER, "/", dbfilename))
    if (!is.data.frame(temptable) | nrow(temptable) == 0){
      ...
      temptable <- temptable %>% select(c(outputid, name)) %>% arrange(outputid) %>%
  ...
  if (any(is.na(varformat))){
    if (tempvar %in% c("estimate", "stderr", "cill", "ciul", "lcb", "ucb")){
      ...
      formatnum <- NA
      tempformat <- list(width = 5, digits = NUM_DECIMAL_DIGITS)
    } else if (tempvar == "icc"){
      formatnum <- 2
      tempformat <- list(NA)
  ...
  dat <- besd_read(paste0(OUTPUT_FOLDER, "/", dbfilename))

  if (!besd_object_exists(paste0(tablename, "_CN"))){
    colnum <- 0
  } else{
  ...
  if (nrow(dat) == 0){
    ...
    make_empty_column <- 1
  } else if (any(duplicated(dat$outputid)) %in% TRUE) {
    warning(paste0("Input criteria do not result in rows with unique values of outputid. The column named ",
                    colname, " will be blank."))
    make_empty_column <- 1
  }
  ...
  if (make_empty_column == 1){
    if (nrow(dat) == 0){
      ...
      dat <- dat %>% select(c(outputid))
      dat <- unique(dat)
      dat <- dat %>% arrange(outputid) %>% mutate(tempvar = NA) %>%
        relocate(c(outputid,tempvar), .after = last_col())
      names(dat)[which(names(dat) == "tempvar")] <- colname
      ...
      temptable <- inner_join(temptable, dat, by = "outputid")
    }


    if (!besd_object_exists(paste0(tablename, "_columnlabel"))){
    ...
    if (!besd_object_exists(paste0(tablename, "_formatnum"))){
    ...
    if (!besd_object_exists(paste0(tablename, "_colformat"))){
    ...
    if (noannotate == FALSE){
      ...
      # generate colname
      variabletoget <- rlang::sym(tempvar)
      dat <- dat %>% mutate(changedvar = !!variabletoget)
      # rename the column
      names(dat)[which(names(dat) == "changedvar")] <- colname
    }
    ...
    dat <- dat %>% arrange(outputid) %>%
      select(c(outputid, all_of(colname))) %>%
      relocate(c(outputid, all_of(colname)), .after = last_col())
