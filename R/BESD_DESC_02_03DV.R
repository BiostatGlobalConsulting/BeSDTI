#' Calculate derived variables for BESD_DESC_02
#'
...
# BESD_DESC_02_03DV R version 1.00 - Biostat Global Consulting - 2024-08-08
# *******************************************************************************
# Change log

# Date 			  Version 	Name			      What Changed
# 2024-08-08  1.00      Caitlin Clary   Original R version adapted from v1.00 of
#                                       DESC_02_03DV in vcqiR
# *******************************************************************************

BESD_DESC_02_03DV <- function(VCP = "BESD_DESC_02_03DV", database_id = NA){
  ...
  for (v in seq_along(DESC_02_VARIABLES)){
    ...
    for (l in seq_along(llist)){
      ...
      dat <- dat %>%
        mutate(
          tempvar1 = ifelse((
          is.na(!!va) & (str_to_upper(DESC_02_DENOMINATOR) == "RESPONDED") &
            psweight > 0 & !is.na(psweight)),
          NA, tempvar1))
    ...
    besd_log_comment(VCP, 3, "Global", paste0("Global value DESC_02_LVL_COUNT_", vcounter, " is ", lcounter-1))
    ...
    if (besd_object_exists("DESC_02_N_SUBTOTALS")){
        ...
        for(i in 1:DESC_02_N_SUBTOTALS){
          ...
          for (j in seq_along(sublevel)){
            dat <- dat %>% mutate(tempvar2 = ifelse(
              ((!!va == sublevel[j]) %in% TRUE | (is.na(!!va) & is.na(sublevel[j]))) & psweight > 0 & !is.na(psweight),
              1,tempvar2))
          } # end of j loop
      ...
      besd_log_comment(VCP, 3, "Global", paste0("Global value DESC_02_ST_COUNT_", vcounter, " is ", DESC_02_N_SUBTOTALS))
  ...
  # saveRDS(dat, file = paste0(OUTPUT_FOLDER, "/DESC_02_", ANALYSIS_COUNTER, "_", DESC_02_COUNTER,".rds"))

  if (!is.na(database_id)){
    saveRDS(dat,
            file = paste0(OUTPUT_FOLDER, "/DESC_02_", ANALYSIS_COUNTER,
                          "_", database_id, ".rds"))
  } else {
    saveRDS(dat,
            file = paste0(OUTPUT_FOLDER, "/DESC_02_", ANALYSIS_COUNTER,
