...
# BESD_DESC_03_03DV R version 1.00 - Biostat Global Consulting - 2024-09-09
# *******************************************************************************
# Change log
...
BESD_DESC_03_03DV <- function(VCP = "BESD_DESC_03_03DV",
                              database_id = NA){
  besd_log_comment(VCP, 5, "Flow", "Starting")

  if (is.na(database_id)){
    dat <- besd_read(
      paste0(OUTPUT_FOLDER, "/DESC_03_", ANALYSIS_COUNTER, "_", DESC_03_COUNTER, ".rds"))
  } else {
    dat <- besd_read(
      paste0(OUTPUT_FOLDER, "/DESC_03_", ANALYSIS_COUNTER, "_", database_id, ".rds"))
  }

  # dat <- besd_read(paste0(
  #   OUTPUT_FOLDER, "/DESC_03_", ANALYSIS_COUNTER, "_", DESC_03_COUNTER, ".rds"))
  ...
  if (besd_object_exists("DESC_03_N_SUBTOTALS")){
    if (DESC_03_N_SUBTOTALS != 0){
      for (i in 1:DESC_03_N_SUBTOTALS){
        ...
        if (i == 1){
          dat <- dat %>%
            relocate(paste0("desc03_", vcounter, "_st", i),
    ...
    besd_log_comment(VCP, 3, "Global",
                     paste0("Global value DESC_03_ST_COUNT_", vcounter,
                            " is ", DESC_03_N_SUBTOTALS))
  ...
  # saveRDS(dat, file = paste0(OUTPUT_FOLDER, "/DESC_03_", ANALYSIS_COUNTER, "_",
  #                            DESC_03_COUNTER, ".rds"))

  if (!is.na(database_id)){
    saveRDS(dat,
            file = paste0(OUTPUT_FOLDER, "/DESC_03_", ANALYSIS_COUNTER,
                          "_", database_id, ".rds"))
  } else {
    saveRDS(dat,
            file = paste0(OUTPUT_FOLDER, "/DESC_03_", ANALYSIS_COUNTER,
