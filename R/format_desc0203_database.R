...
# format_desc0203_database R version 1.00 - Biostat Global Consulting - 2024-09-09
# *******************************************************************************
# Change log

# Date 			  Version 	Name			      What Changed
# 2024-09-09  1.00      Caitlin Clary   Original R version adapted from v1.01 of
#                                       format_desc0203_database in vcqiR
# *******************************************************************************

format_desc0203_database <- function(
    VCP = "format_desc0203_database",
    i, db, wtd, indicator, datname, vid) {

  dat2 <- besd_read(datname)
  ...
  if (besd_object_exists(paste0(indicator, "_LIST_N_BEFORE_PCT"))){
    ...
    if (str_to_upper(nbp) == "YES"){
      ...
      assign(
        paste0("n", i),
        paste0(pctl, " ", language_string(language_use = language_use, str = "OS_2")),
        envir = .GlobalEnv)
      #(N)
    }
  }

  if (besd_object_exists(paste0(indicator, "_LIST_NWTD_BEFORE_PCT"))){
    ...
    if (wtd == 1 & str_to_upper(nwbp) == "YES"){
      ...
      assign(paste0("nwtd", i),
        paste0(pctl, " ", language_string(language_use = language_use, str = "OS_3")),
        envir = .GlobalEnv) # (Weighted N)

    }
  }

  if (besd_object_exists(paste0(indicator, "_SUPPRESS_CI_OUTPUT"))){
  ...
  dat2 <- dat2 %>% select(c(level, name, outputid, outputname, all_of(vartokeep)))

  # Now rename variables for formatting purposes in make_table_column
  ...
  saveRDS(dat2, file = paste0(OUTPUT_FOLDER, "/", db, "_", i, ".rds"))
